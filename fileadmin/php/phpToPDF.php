...
class phpToPDF extends FPDF
{
	...
	function AddPage($orientation='', $size='', $rotation = '') {
		parent::AddPage($orientation, '', 0);
		if($this->_numbering)
			$this->_numPageNum++;
	}
			...
			for ($j=$i ; $j<$i+$nbCol ; $j++)
			{
				...
				if (sscanf($data[$k]['TEXT'], "[%[a-zA-Z]]%s", $balise, $reste) != 0)
				{
                    ...
                    if ( (strpos($balise, "I")===FALSE) && (strpos($balise, "B")===FALSE) && (strpos($balise, "U")===FALSE)
                      ...
                      && (strpos($balise, "W")===FALSE) && (strpos($balise, "G")===FALSE) )
					...
					else
					{
                        ...
                        else $data[$k]['T_COLOR'] = array(255,0,0); 
                        ...
                        if (strpos($balise, "Z") === FALSE) ;//Fond jaune pas trop claire
                        else $data[$k]['BG_COLOR'] = array(245, 245, 150);                        
